...
object PhoneCheck {
    ...
    fun run(context: Context): List<Item> {
        ...
        if (!HomeRole.isDefault(context)) out += Item(
            "home", Level.PROBLEM, "Book Launcher isn't your home screen",
        ...
        if (Build.VERSION.SDK_INT >= 28 && am?.isBackgroundRestricted == true) out += Item(
            ...
            "Android is limiting Book Launcher in the background, so downloads and Now listening can stop. Set battery use to Unrestricted.",
        ...
        if (pm != null && !pm.isIgnoringBatteryOptimizations(pkg) && isAggressiveBrand) out += Item(
            "battery", Level.RECOMMENDED, "Don't let ${Build.MANUFACTURER.replaceFirstChar { it.uppercase() }} put Book Launcher to sleep",
            "Find Book Launcher in the list and choose \"Don't optimise\" (or Unrestricted). Home then opens instantly and Now listening stays on.",
        ) { open(it, Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS), appDetails(it)) }
    ...
    private fun autostart(context: Context): Item? {
        val (title, hint, pages) = when {
            isXiaomi -> Triple("Allow Autostart", "Turn on Autostart for Book Launcher.",
                listOf(component("com.miui.securitycenter", "com.miui.permcenter.autostart.AutoStartManagementActivity")))
            isOppoFamily -> Triple("Allow Auto launch", "Turn on Auto launch for Book Launcher.", listOf(
            ...
            isVivo -> Triple("Allow background power use", "Battery → Background power consumption → allow Book Launcher.",
                listOf(component("com.vivo.permissionmanager", "com.vivo.permissionmanager.activity.BgStartUpManagerActivity")))
            isHuawei -> Triple("Manage app launch manually", "Phone manager → App launch → Book Launcher → Manage manually, all on.",
                listOf(component("com.huawei.systemmanager", "com.huawei.systemmanager.startupmgr.ui.StartupNormalAppListActivity")))
            isSamsung -> Triple("Keep Book Launcher out of sleeping apps",
                "Settings → Battery → Background usage limits → make sure Book Launcher isn't in Sleeping or Deep sleeping apps.",
                emptyList())
            else -> return null
        }
