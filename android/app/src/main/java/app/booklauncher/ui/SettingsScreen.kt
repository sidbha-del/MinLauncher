...
import app.booklauncher.data.HomeRole
...
class SettingsScreen(host: HomeActivity) : Screen(host) {
    ...
    override fun build(): View {
        ...
        list.addView(ui.text(
            "Shows what Audible, Libby and other audiobook apps are playing, with play and skip. " +
                "Android files this under notification access; Book Launcher only reads what's playing and never your notifications.",
            13f, p.soft,
        ).apply { setLineSpacing(0f, 1.3f) }.also { ui.margins(it, 18, 10, 18, 6) })

        list.addView(ui.group("Focus"))
        val paused = app.booklauncher.data.Focus.distracting(prefs, host.catalog).size
        ...
        val usage = app.booklauncher.data.Focus.hasUsageAccess(host)
        ...
        val checks = app.booklauncher.data.PhoneCheck.run(host)
        if (checks.isEmpty()) {
            list.addView(ui.row("Everything looks good", "✓"))
        }
        for (c in checks) {
            list.addView(ui.row(c.title, if (c.level == app.booklauncher.data.PhoneCheck.Level.PROBLEM) "Fix" else "Check") {
        ...
        list.addView(ui.row("Find covers & details online", if (prefs.onlineMeta) "On" else "Off") {
            ...
            if (prefs.onlineMeta) app.booklauncher.data.OnlineMeta.lookupPending(host)
        })
        ...
        list.addView(ui.row("Version", app.booklauncher.BuildConfig.VERSION_NAME))
    ...
    private fun openMediaAccessSettings() {
        if (!NowPlaying.get(host).openAccessSettings(host)) {
            Toast.makeText(host, "Open Settings → Notification access to allow Book Launcher", Toast.LENGTH_LONG).show()
        }
    }
...
class SetupScreen(host: HomeActivity) : Screen(host) {
    ...
    override fun build(): View {
        val prefs = host.prefs
        val col = ui.vertical()
        col.addView(ui.topBar("Book Launcher", "", "Setup"))
