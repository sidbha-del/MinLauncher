...
object TimeStats {
    ...
    private fun fill(host: HomeActivity, table: LinearLayout, days: List<Focus.Day>) {
        ...
        table.addView(ui.text(
            "Reading counts ReadFirst's reader, time in reading apps like Kindle, and audiobook listening in apps like Audible (screen on or off, once Now listening is allowed). Long-press any app to change whether it counts as work or pauses as distracting.",
            12.5f, p.soft,
    ...
    fun openUsageAccess(context: Context) {
        ...
        if (!app.readfirst.data.PhoneCheck.open(context, direct, Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS))) {
...
object HomeNotice {
    fun build(host: HomeActivity): View? {
        val ui = host.ui
        val p = ui.p
        val problem = app.readfirst.data.PhoneCheck.run(host)
            .firstOrNull { it.level == app.readfirst.data.PhoneCheck.Level.PROBLEM && it.id !in host.laterNotices }
        if (problem != null) {
