...
class LibraryScreen(host: HomeActivity) : Screen(host) {
    ...
    private fun otherReadingApps(): View? {
        val apps = app.readfirst.data.ReaderApps.installed(host.catalog, host.prefs.hidden)
    ...
    private fun stackOf(books: List<app.readfirst.data.BookEntry>, current: String?): View {
...
object BookActions {
    fun show(host: HomeActivity, bookArg: app.readfirst.data.BookEntry) {
        val ui = host.ui
        val p = ui.p
        ...
        val sub = listOf(book.author, book.format.name, "${app.readfirst.book.Progress.percent(book.progress)}%").filter { it.isNotBlank() }
    ...
    private fun details(host: HomeActivity, book: app.readfirst.data.BookEntry) {
        val ui = host.ui
